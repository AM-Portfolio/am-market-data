...
class AMMarketDataStreamingAPI {
  ...
  AMMarketDataStreamingAPI({String? url})
    : _url = url ?? 'ws://localhost:8092/ws/market-data-stream';


  /// Method to establish a WebSocket connection
  Future<void> connect() async {
    if (_channel != null) {
      print('Already connected to AM Market Data Streaming API server');
      return;
    }
    try {
      final wsUrl = Uri.parse(_url);
      _channel = WebSocketChannel.connect(wsUrl);
      print('Connected to AM Market Data Streaming API server');
      ...
      _channel?.stream.listen(
        ...
        onDone: () {
          _channel = null;
          print('Disconnected from AM Market Data Streaming API server');
        },
      );
    } catch (error) {
